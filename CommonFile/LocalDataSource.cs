...
using Fisbank.Cbs.CommonLib;
using Fisbank.Cbs.DataMemory;
using Fisbank.Cbs.Diagnostics;

namespace Fisbank.Cbs.BusinessLogic
{
    public class LocalDataSource
    {
        public static List<T> GetListDataFromProcedure<T>(string connectionString, string procName, params object[] paramsProcs) where T : class, new()
        {
            ...
            {
                ErrorLog.Log(ex + "_" + procName + "_" + connectionString);
                return null;
            }
        }
        ...
        public static object[] ConvertObjectToArrayParam<T>(T data) where T : class, new()
        {
            try
            {
                ...
                //}
                //return values;
                return GetListPropertiesFormObjects(props, data);
            }
            catch (Exception ex)
            {
                ErrorLog.Log(ex);
        ...
        public static object[] ConvertBaseClassToDerivedClassArrayParam<T>(T data) where T : class, new()
        {
            try
            {
        ...
        private static object[] GetListPropertiesFormObjects(PropertyDescriptorCollection props, object data)
        {
            try
            {
                object[] values = new object[props.Count];
        ...
        public static string GetReturnValueFromProcedure(string connectionString, string procName, params object[] paramsProcs)
        {
            try
            {
                string res;
                if (paramsProcs.IsNull())
                    res = SqlHelper.ExecuteScalar(connectionString, CommandType.StoredProcedure, procName).ToString();
                else
                    res = SqlHelper.ExecuteScalar(connectionString, procName, paramsProcs).ToString();
                return res;
            }
            catch (Exception ex)
            {
        ...
        public static string GetReturnValueFromCommand(string connectionString, string sqlCommand, SqlParameter[] sendingParams)
        {
            try
            {
                return SqlHelper.ExecuteScalar(connectionString, CommandType.Text, sqlCommand, sendingParams).ToString();
            }
            catch (Exception ex)
            {
                ErrorLog.Log(ex);
